...
namespace CloudMagic.Rotation
{
    public class BalanceDruidNoStarfall : CombatRoutine
    {
        ...
        {
            MessageBox.Show("Welcome to Balance Druid by Scotishdwarf r31.\n\nMy talent build : 3,1,3,1,3,3,3.\n\nNoteworthy things :\n- This build is for those who want have 100% manual control on Starfall");
            ...
            var lblStarfallMacroText = new Label { Text = "Starfall Macro Disabled", Size = new Size(200, 13), Left = 312, Top = 44 };
        ...
        public override void Pulse()
        {
            if (combatRoutine.Type == RotationType.SingleTarget)
            {
                ...
                if (WoW.IsInCombat && pullwatch.ElapsedMilliseconds < 15000 && UseCooldowns)
                {
                    ...
                    if (WoW.PlayerHasBuff("CelestialAlignment") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
						{
                        // KBW if in use
                        if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                        {
                    ...
                    // Under Incarnation
                    if (Incarnation && WoW.PlayerHasBuff("Incarnation") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
					{
                        // KBW if in use
                        if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                        {
				...
				// Under Celestial Alignment
				if (WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerHasBuff("Moonkin") && WoW.PlayerHasBuff("CelestialAlignment") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
				{
                    // KBW if in use
                    if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                    {
				...
				// Under Incarnation
				if (Incarnation && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerHasBuff("Moonkin")&& WoW.PlayerHasBuff("Incarnation") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
				{
                    // KBW if in use
                    if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                    {
                ...
                if (WoW.IsInCombat && WoW.HasTarget&& WoW.TargetIsEnemy&&WoW.PlayerHasBuff("Moonkin") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
                {
                    // Owlkin Frenzy
            ...
            if (combatRoutine.Type == RotationType.AOE)
            {
                ...
                if (SouloftheForest && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerHasBuff("Moonkin") && WoW.IsSpellInRange("LStrike") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
                {
                    // KBW if in use
                    if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                    {
                    ...
                    // && WoW.TargetHasDebuff("StarfallT")
                    if (WoW.CanCast("Starsurge") && WoW.CurrentAstralPower >= 40 && (WoW.PlayerHasBuff("Incarnation") || WoW.PlayerHasBuff("CelestialAlignment")))
                    {
                        WoW.CastSpell("Starsurge");
                        return;
                    }
                    // && WoW.PlayerHasBuff("StarfallP")
                    if (WoW.CanCast("LStrike"))
                    {
                        WoW.CastSpell("LStrike");
				...
				if (!SouloftheForest && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerHasBuff("Moonkin") && WoW.PlayerHasBuff("CelestialAlignment") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
				{
                    // KBW if in use
                    if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                    {
				...
				if (!SouloftheForest && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerHasBuff("Moonkin") && WoW.PlayerHasBuff("Incarnation") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
				{
                    // KBW if in use
                    if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                    {
                ...
                if (!SouloftheForest && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerHasBuff("Moonkin") && (!WoW.IsMoving || WoW.PlayerHasBuff("StarfallP")))
                {
                    // KBW if in use
                    if (KBW && !WoW.ItemOnCooldown("KBW") && WoW.IsSpellInRange("LStrike"))
                    {
