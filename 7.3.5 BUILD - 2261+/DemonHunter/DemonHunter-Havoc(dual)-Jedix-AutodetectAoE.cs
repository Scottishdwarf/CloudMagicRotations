	...
	private static readonly Stopwatch coolDownStopWatch = new Stopwatch();
        ...
        public override void Pulse()
        {
            if (combatRoutine.Type == RotationType.SingleTarget) // Do Single Target Stuff here
            {
                ...
                {
					if (WoW.CanCast("Consume Magic") && WoW.TargetIsCastingAndSpellIsInterruptible && WoW.TargetPercentCast >= 60 && WoW.IsSpellInRange("Chaos Strike"))
					{	
						WoW.CastSpell("Consume Magic");
						return;
					}
					if (WoW.CanCast("Arcane Torrent") && WoW.IsSpellOnCooldown("Consume Magic") && WoW.TargetIsCastingAndSpellIsInterruptible && WoW.TargetPercentCast >= 60 && WoW.IsSpellInRange("Chaos Strike"))
					{	
                    ...
                    if (WoW.PlayerHasBuff("Metamorphosis"))
                    {
                        ...
                        if (WoW.CanCast("Death Sweep") && WoW.IsSpellInRange("Chaos Strike") && WoW.Fury >= 35)
                        {
                        ...
                        if (WoW.CanCast("Eye Beam") && WoW.Fury >= 30 && WoW.IsSpellInRange("Chaos Strike"))
                        {																
...
Buff,162264,Metamorphosis
Buff,247938,Chaos Blades
Buff,208628,Momentum
Range,162794,Chaos Strike
Range,185123,Throw Glaive
Charge,211053,Fel Barrage
*/
